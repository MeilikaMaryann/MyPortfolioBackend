...
const educationService = require("./service/EducationalBG");
...
app.get('/api/getField/:id', async (req, res) => {
  try {
    const result = await professionalService.getField(11);
    if (result) {
...
app.get('/api/getInterest/:id', async (req, res) => {
  try {
    const result = await interestService.getInterest(11);
    if (result) {
      res.status(200).send(result);
    } else {
...
app.get('/api/get_recommendation/:id', async (req, res) => {
  try {
    const result = await recommendationService.getRecommendation(12);
    if (result) {
      res.status(200).send(result);
    } else {
...
app.get('/api/get_certification/:id', async (req, res) => {
    console.log(req.params.id);
    const result = await certificationService.getCertification(12);
    if (result) {
        console.log(result);
        res.send(result);
    }
